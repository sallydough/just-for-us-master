...
const Activities = React.forwardRef((props, ref) => {
  ...
  const [isApologiesModalOpen, setIsApologiesModalOpen] = useState(false);
  const [cardIndex, setCardIndex] = useState(0);
  ...
  useEffect(() => {
    events.forEach((event) => {
      ...
      const isToday =
        eventStartDate.toDateString() === currentTime.toDateString();

      // Calculate the time difference in milliseconds
      const timeDifference = eventStartDate.getTime() - currentTime.getTime();

      // Check if the event is scheduled within the next 10 minutes on the present day
      const isWithin10Minutes =
        isToday && timeDifference > 0 && timeDifference <= 10 * 60 * 1000;

      // If the event is scheduled within the next 10 minutes on the present day,
      // set up the reminder
      if (isWithin10Minutes) {
        const reminderTimeout = setTimeout(() => {
          ...
          setSelectedEvent(event);
        }, timeDifference - 10 * 60 * 1000); // Trigger the reminder 10 minutes before the event
  ...
  return (
    <>
      ...
      {/* Dynamic reminder modal */}
      {isReminderModalOpen && selectedEvent && (
        <div className="overlay">
          <div className="modal">
            <div>
              <h1>Reminder</h1>
              <p>Your event "{selectedEvent.item}" starts in 10 minutes.</p>
      ...
      {/* Selected event modal for Join Now */}
      {isJoinNowModalOpen && selectedEvent && (
        <div className="overlay">
          <div className="modal">
            <div>
      ...
      {/* Modal for when the event is not available at present */}
      {isApologiesModalOpen && selectedEvent && (
        <div className="overlay">
          <div className="modal">
            <div>
              <h1>{selectedEvent.item}</h1>
              <p>Apologies, the event is not available at present.</p>
              <button onClick={closeModal}>Close</button>
            </div>
          </div>
        </div>
